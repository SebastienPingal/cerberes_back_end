...
export default class user {
  ...
  static async find_one_by_uuid(uuid: string) {
    try {
      const user = await User.findOne({ where: { User_contact_uuid: uuid } }) as IUser
  ...
  static async find_one_by_id(id: number) {
    try {
      const user = await User.findOne({ where: { User_id: id } }) as IUser
      delete user.User_password
      return user
    } catch (error) {
      throw new Error('Unable to find user')
  ...
  static async find_one_by_email(email: string) {
    try {
      const user = await User.findOne({ where: { User_email: email } }) as IUser
      delete user.User_password
    } catch (error) {
      throw new Error('Unable to find user')
    }
